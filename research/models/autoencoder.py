...
def generator(batch_size, gf_dim, ch, rows, cols):
    ...
    model.add(Dense(gf_dim * 8 * rows[0] * cols[0], batch_input_shape=(
    batch_size, z_dim), name="g_h0_lin", init=normal))
...
def get_model(sess, image_shape=(80, 160, 3), gf_dim=64, df_dim=64, batch_size=64,
    ...
    with tf.variable_scope(name), tf.device("/gpu:{}".format(gpu)):
        ...
        kl_loss = tf.reduce_mean(-E_logsigma + .5 * (
    ...
    def train_d(images, z, counter, sess=sess):
        ...
        outs = sess.run(outputs + updates, feed_dict={Img: images, Z: z, Z2: z2,
                                                      K.learning_phase(): 1})
