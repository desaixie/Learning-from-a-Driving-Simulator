...
class Deconv2D(Layer):
    ...
    def call(self, x, mask=None):
        ...
        deconv_out = tf.nn.conv2d_transpose(
            x, self.W, output_shape=output_shape, strides=[1, self.subsample[0], self.subsample[1],
                                                           1])
...
class CondDreamyRNN(Recurrent):
    ...
    def dream(self, x, states):
        ...
        prev_x = K.concatenate([controls,
                                tf.stop_gradient(K.dot(prev_st, self.V) + self.ext_b)], axis=1)
        B_U = states[1]
        B_W = states[2]
        h = K.dot(prev_x * B_W, self.W) + self.b
    ...
    def call(self, x, mask=None):
        ...
        last_output, outputs_0, states = K.rnn(self.step, preprocessed_input[:,
                                                          :input_shape[1] - self.output_length, :],
                                               initial_states,
                                               go_backwards=self.go_backwards,
                                               mask=mask,
                                               constants=constants,
                                               unroll=self.unroll,
                                               input_length=input_shape[1])
        last_output, outputs, states = K.rnn(self.dream, preprocessed_input[:,
                                                         input_shape[1] - self.output_length:, :],
                                             states, go_backwards=self.go_backwards,
                                             mask=mask,
                                             constants=constants,
